...
def morning():
    ...
    prompt = (
        ...
        "mail) as 'sender — gist'. If he has ALREADY replied (check the thread, not just the inbox copy), "
        "say so in the gist or drop it: an answered message is news, not a task. Then today's calendar: "
        "call list_calendars, then get_events (today) "
...
def review():
    ...
    prompt = (
        ...
        "BEFORE you flag anything as owed, CHECK WHETHER HE HAS ALREADY ANSWERED IT. An inbox message "
        "tells you a thread exists, not whose turn it is. For EVERY candidate, call "
        "get_gmail_thread_content on its thread_id with include_analysis=true and read the ownership "
        "verdict: if the last message in the thread is from Mikael, he does NOT owe a reply and you must "
        "drop it. Searching 'in:sent' for the same correspondent is a good cross-check. Never decide "
        "from the received message alone.\n"
        "This is the failure that prompted the rule (operator, 2026-08-29): a correspondent was posted "
        "as owing a reply hours after he had already sent one. Telling him to answer mail he has answered "
        "is worse than silence, because it costs him the time to go and check, and it teaches him not to "
        "trust the list.\n"
