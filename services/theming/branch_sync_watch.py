...
# Who counts as a machine. The estate's own PRs are authored by odinlake-ai; the repo's totolo
# workflows use GITHUB_TOKEN, so theirs come from github-actions[bot]. Auto-merge applies to both,
# and to nothing else — a human's PR is never merged by this.
PR_AUTHORS = set(filter(None, os.environ.get(
    "BRANCH_SYNC_PR_AUTHORS", "odinlake-ai,github-actions[bot],app/github-actions").split(",")))
PR_TITLE_PREFIX = os.environ.get("BRANCH_SYNC_TITLE_PREFIX", "M4: ")
# Heads that are machine-managed by name. The title prefix covers everything raised from now on;
# these cover what was raised before it existed, and are the branch names the repo's own workflows
# hard-code (see BOT_BRANCHES in .github/scripts/branch_sync.py).
MACHINE_HEADS = ("ai-feature-sync-", "totolo-reformat", "totolo-merge-proposed")
...
def merge_approved_prs():
    """Merge any open MACHINE-RAISED PR that a human has APPROVED. Returns one line per merge.
    ...
    Scope is deliberately narrow: opened by a machine account, carrying the "M4: " title prefix
    every autogenerated PR now uses (or an ai-feature-sync-* head, for ones raised before that
    convention), and only once reviewDecision is APPROVED. That covers the estate's sync PRs and the
    repo's two totolo workflow PRs, and nothing a person raised. AGENTS.md rule 3 says do not merge your own PR; the
    ...
    """
    rc, out, err = gh_raw(["pr", "list", "--repo", REPO, "--state", "open", "--limit", "30",
                           "--json", "number,headRefName,baseRefName,reviewDecision,author,title"])
    if rc != 0:
    ...
    for pr in prs:
        ...
        machine = ((pr.get("title") or "").startswith(PR_TITLE_PREFIX)
                   or any(head.startswith(h) for h in MACHINE_HEADS))
        if not machine:
            continue
        # Belt and braces: the title prefix is a convention a human could type by accident, the
        # author is a fact. Both must hold.
        if (pr.get("author") or {}).get("login") not in PR_AUTHORS:
            continue
